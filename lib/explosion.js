...
class Explosion {
  ...
  checkForDeadPlayers() {
    ...
    if (playerOne.alive === false)  {
      this.game.endGame('player two')
    } else if (playerTwo.alive === false) {
      this.game.endGame('player one')
    }
  }
  ...
  checkForPlayers(x, y, width, height) {
    const playerOne = this.game.firstPlayer   
    const playerTwo = this.game.secondPlayer   
    if ((x <= playerOne.x <= (x + width)) && (y <= playerOne.y <= (y + height))) {
      playerOne.alive = false
    } 
    if ((x <= playerTwo.x <= (x + width)) && (y <= playerTwo.y <= (y + height))) {
