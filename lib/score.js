class Score {
  ...
  placeHighestScore() {
    ...
    const node = document.createTextNode(`Highest Score: ${localStorage.getItem('score')}`)
  ...
  update() {
    this.firstPlayerNode.textContent = `Player One Score: ${this.firstPlayer/6}`
    this.secondPlayerNode.textContent = `Player Two Score: ${this.secondPlayer/6}`
  }
}
