class Player {
  ...
  rightCollide(block) {
    return (block.x -3 <= (this.x + this.width) && (this.x + this.width) <= block.x) &&
      ((block.y < this.y && this.y < block.y + block.height) ||
      (block.y < this.y + this.height && this.y + this.height < block.y + block.height))
  }

  leftCollide(block) {
    return (this.x <= (block.x + block.width + 3) && this.x >= (block.x + block.width)) &&
      ((block.y < this.y && this.y < block.y + block.height) ||
      (block.y < this.y + this.height && this.y + this.height < block.y + block.height))
  }

  downCollide(block) {
    return ((this.y + this.height) <= block.y && (this.y + this.height) >= (block.y -3)) &&
      ((block.x < this.x && this.x < block.x + block.width) ||
      (block.x < this.x + this.width && this.x + this.width < block.x + block.width))
  }

  upCollide(block) {
    return (this.y <= (block.y + block.height + 3) && this.y >= (block.y + block.height)) &&
      ((block.x < this.x && this.x < block.x + block.width) ||
      (block.x < this.x + this.width && this.x + this.width < block.x + block.width))
  }
